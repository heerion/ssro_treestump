# 연표 편집기 저장 중계 서버 (Cloudflare Worker)

`admin/timeline.html` 편집기에서 **저장** 버튼을 누르면, 이 작은 서버가 대신
GitHub에 커밋합니다. 위원들은 GitHub 계정도, 토큰도 몰라도 되고, 공유받은
...
5. 에디터에 있던 내용을 전부 지우고, 이 폴더의 `save-timeline.js` 파일
   내용을 그대로 붙여넣기
6. **Deploy** 로 저장
...
1. Worker 화면 위쪽에 나오는 주소를 복사합니다.
   (`https://ssro-timeline-save.<내계정>.workers.dev` 형태)
2. 이 저장소의 `admin/timeline-admin.js` 파일을 열어 맨 위쪽
   `SAVE_ENDPOINT` 값에 그 주소를 넣고 저장·커밋·푸시합니다.

```js
const SAVE_ENDPOINT = "https://ssro-timeline-save.내계정.workers.dev";
```

3. 사이트가 다시 배포되면(보통 1분 안팎), `admin/timeline.html` 을 열어
...
- **"GitHub에서 파일을 읽지 못했습니다" / "GitHub 저장에 실패했습니다"** —
  토큰이 이 저장소에 Contents: Read and write 권한으로 되어 있는지,
  `GITHUB_OWNER`/`GITHUB_REPO`/`GITHUB_BRANCH` 값이 정확한지 확인하세요.
  토큰 유효 기간이 지났다면 새로 만들어 `GITHUB_TOKEN` 을 갱신하세요.
